...
1. Developers *MUST* pass the required CI checks for each PR. The checks enforce code quality with a formatter and linter ([ruff](https://docs.astral.sh/ruff/)) and a type checker ([mypy](https://mypy.readthedocs.io/)).
...
```bash
make code-format   # ruff format + lint (auto-fix)
make code-lint     # ruff lint (check only)
make code-typing   # mypy type checking
make mdformat      # mdformat on all Markdown files
```

Note that autogenerated files under `trestle/oscal` are excluded from ruff linting and formatting.
