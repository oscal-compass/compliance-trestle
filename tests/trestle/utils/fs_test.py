...
def test_should_ignore() -> None:
    ...
    assert ModelIO._should_ignore('.test') is True
    assert ModelIO._should_ignore('_test') is True
    assert ModelIO._should_ignore('__test') is True
    assert ModelIO._should_ignore('test') is False
...
def test_get_relative_model_type(tmp_path: pathlib.Path) -> None:
    ...
    with pytest.raises(TrestleError):
        ModelIO.get_relative_model_type(pathlib.Path('./'))
    ...
    with pytest.raises(TrestleError):
        ModelIO.get_relative_model_type(catalogs_dir)

    assert ModelIO.get_relative_model_type(mycatalog_dir) == (catalog.Catalog, 'catalog')
    assert ModelIO.get_relative_model_type(mycatalog_dir / 'catalog.json') == (catalog.Catalog, 'catalog')
    assert ModelIO.get_relative_model_type(catalog_dir / 'back-matter.json'
                                           ) == (common.BackMatter, 'catalog.back-matter')
    assert ModelIO.get_relative_model_type(catalog_dir / 'metadata.yaml') == (common.Metadata, 'catalog.metadata')
    assert ModelIO.get_relative_model_type(metadata_dir) == (common.Metadata, 'catalog.metadata')
    assert ModelIO.get_relative_model_type(roles_dir) == (List[common.Role], 'catalog.metadata.roles')
    (type_, element) = ModelIO.get_relative_model_type(roles_dir)
    assert cutils.get_origin(type_) == list
    assert element == 'catalog.metadata.roles'
    assert ModelIO.get_relative_model_type(roles_dir / '00000__role.json'
                                           ) == (common.Role, 'catalog.metadata.roles.role')
    model_type, full_alias = ModelIO.get_relative_model_type(rps_dir)
    assert model_type == List[common.ResponsibleParty]
    assert full_alias == 'catalog.metadata.responsible-parties'
    assert ModelIO.get_relative_model_type(
        rps_dir / 'creator__responsible-party.json'
    ) == (common.ResponsibleParty, 'catalog.metadata.responsible-parties.responsible-party')
    (type_, element) = ModelIO.get_relative_model_type(props_dir)
    assert cutils.get_origin(type_) == list
    assert cutils.get_inner_type(type_) == common.Property
    assert element == 'catalog.metadata.props'
    (expected_type, expected_json_path) = ModelIO.get_relative_model_type(
    ...
    assert cutils.get_origin(type_) == list
    assert ModelIO.get_relative_model_type(groups_dir / f'00000{const.IDX_SEP}group.json'
                                           ) == (catalog.Group, 'catalog.groups.group')
    assert ModelIO.get_relative_model_type(group_dir) == (catalog.Group, 'catalog.groups.group')
    assert ModelIO.get_relative_model_type(controls_dir / f'00000{const.IDX_SEP}control.json'
...
def test_extract_alias() -> None:
    ...
    assert ModelIO._extract_alias(pathlib.Path('catalog').name) == 'catalog'
    assert ModelIO._extract_alias(pathlib.Path('/tmp/catalog').name) == 'catalog'
    assert ModelIO._extract_alias(pathlib.Path('/catalogs/mycatalog/catalog.json').name) == 'catalog'
    assert ModelIO._extract_alias(pathlib.Path('/catalogs/mycatalog/catalog.yaml').name) == 'catalog'
    assert ModelIO._extract_alias(pathlib.Path('responsible-parties').name) == 'responsible-parties'
    assert ModelIO._extract_alias(pathlib.Path('responsible-parties.json').name) == 'responsible-parties'
    assert ModelIO._extract_alias(pathlib.Path('/roles').name) == 'roles'
    assert ModelIO._extract_alias(pathlib.Path('/roles/roles.json').name) == 'roles'
    assert ModelIO._extract_alias(pathlib.Path(f'/roles/00000{const.IDX_SEP}role.json').name) == 'role'
    assert ModelIO._extract_alias(
...
def test_get_stripped_model_type(tmp_path: pathlib.Path) -> None:
    ...
    with pytest.raises(TrestleError):
        ModelIO.get_stripped_model_type(tmp_path / 'invalidpath', tmp_path)

    with pytest.raises(TrestleError):
        ModelIO.get_stripped_model_type(tmp_path, tmp_path)

    create_sample_catalog_project(tmp_path)

    catalogs_dir = tmp_path / 'catalogs'
    with pytest.raises(TrestleError):
        ModelIO.get_stripped_model_type(catalogs_dir, tmp_path)
    ...
    mycatalog_dir = catalogs_dir / 'mycatalog'
    stripped_catalog = ModelIO.get_stripped_model_type(mycatalog_dir, tmp_path)
    alias_to_field_map = stripped_catalog[0].alias_to_field_map()
    check_stripped_catalog()

    stripped_catalog = ModelIO.get_stripped_model_type(mycatalog_dir / 'catalog.json', tmp_path)
    alias_to_field_map = stripped_catalog[0].alias_to_field_map()
    check_stripped_catalog()
    ...
    catalog_dir = mycatalog_dir / 'catalog'
    metadata_dir = catalog_dir / 'metadata'
    stripped_catalog = ModelIO.get_stripped_model_type(metadata_dir, tmp_path)
    alias_to_field_map = stripped_catalog[0].alias_to_field_map()
    check_stripped_metadata(alias_to_field_map)

    stripped_catalog = ModelIO.get_stripped_model_type(catalog_dir / 'metadata.json', tmp_path)
    alias_to_field_map = stripped_catalog[0].alias_to_field_map()
    check_stripped_metadata(alias_to_field_map)

    groups_dir = catalog_dir / 'groups'
    stripped_catalog = ModelIO.get_stripped_model_type(groups_dir, tmp_path)
    ...
    stripped_catalog = ModelIO.get_stripped_model_type(groups_dir / f'00000{const.IDX_SEP}group', tmp_path)
    alias_to_field_map = stripped_catalog[0].alias_to_field_map()
    check_stripped_group()

    stripped_catalog = ModelIO.get_stripped_model_type(groups_dir / f'00000{const.IDX_SEP}group.json', tmp_path)
    alias_to_field_map = stripped_catalog[0].alias_to_field_map()
    check_stripped_group()
...
def test_get_singular_alias() -> None:
    ...
    assert ModelIO.get_singular_alias(alias_path='catalog') == 'catalog'

    # Not fullpath. It should be 'catalog.metadata' instead
    with pytest.raises(TrestleError):
        ModelIO.get_singular_alias(alias_path='metadata.something')

    # Invalid alias_path
    with pytest.raises(TrestleError):
        ModelIO.get_singular_alias(alias_path='invalid')
    # Invalid alias_path
    with pytest.raises(TrestleError):
        ModelIO.get_singular_alias(alias_path='')

    assert ModelIO.get_singular_alias(alias_path='catalog.metadata.responsible-parties') == 'responsible-party'
    assert ModelIO.get_singular_alias(alias_path='catalog.metadata.responsible-parties.*.props') == 'property'
    assert 'responsible-party' == ModelIO.get_singular_alias(alias_path='catalog.metadata.responsible-parties.*')

    assert 'role' == ModelIO.get_singular_alias(alias_path='catalog.metadata.roles')
    assert 'property' == ModelIO.get_singular_alias(alias_path='catalog.metadata.props')

    assert 'control-implementations' == ModelIO.get_singular_alias(
        alias_path='component-definition.components.control-implementations'
    )
    assert 'control-implementation' == ModelIO.get_singular_alias(
        alias_path='component-definition.components.*.control-implementations'
    )
    assert 'control-implementation' == ModelIO.get_singular_alias(
    ...
    assert '0' == ModelIO.get_singular_alias(alias_path='component-definition.components.0')

    assert 'control' == ModelIO.get_singular_alias(alias_path='catalog.groups.*.controls.*.controls')
...
def test_contextual_get_singular_alias(tmp_path: pathlib.Path, keep_cwd: pathlib.Path) -> None:
    ...
    assert 'responsible-party' == ModelIO.get_singular_alias('catalog.metadata.responsible-parties', rel_dir)
    # Both should work to deal with the case back-matter is already split from the catalog in a separate file
    assert 'resource' == ModelIO.get_singular_alias('catalog.back-matter.resources', rel_dir)
    assert 'resource' == ModelIO.get_singular_alias('back-matter.resources', rel_dir)

    rel_dir = metadata_dir.relative_to(tmp_path)
    with pytest.raises(TrestleError):
        ModelIO.get_singular_alias('metadata.roles')
    alias = ModelIO.get_singular_alias('metadata.roles', rel_dir)
    assert alias == 'role'
    assert 'responsible-party' == ModelIO.get_singular_alias('metadata.responsible-parties.*', rel_dir)

    assert 'property' == ModelIO.get_singular_alias('metadata.responsible-parties.*.props', rel_dir)

    rel_dir = groups_dir.relative_to(tmp_path)
    assert 'control' == ModelIO.get_singular_alias('groups.*.controls.*.controls', rel_dir)

    rel_dir = group_dir.relative_to(tmp_path)
    assert 'control' == ModelIO.get_singular_alias('group.controls.*.controls', rel_dir)
...
def test_get_models_of_type(tmp_trestle_dir) -> None:
    ...
    models = ModelIO.get_models_of_type('catalog', tmp_trestle_dir)
    assert len(models) == 2
    assert 'mycatalog' in models
    assert 'mycatalog2' in models
    all_models = ModelIO.get_all_models(tmp_trestle_dir)
...
def test_model_type_to_model_dir() -> None:
    ...
    assert ModelIO.model_type_to_model_dir('catalog') == 'catalogs'
    try:
        ModelIO.model_type_to_model_dir('foo')
    except Exception:
        pass
    else:
        assert 'test failed'
...
def test_full_path_for_top_level_model(tmp_trestle_dir: pathlib.Path, sample_catalog_minimal: catalog.Catalog) -> None:
    ...
    ModelIO.save_top_level_model(sample_catalog_minimal, tmp_trestle_dir, 'mycat', FileContentType.JSON)
    cat_path = ModelIO.full_path_for_top_level_model(tmp_trestle_dir, 'mycat', catalog.Catalog)
