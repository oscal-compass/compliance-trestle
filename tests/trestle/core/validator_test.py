...
def test_has_no_duplicate_values_generic():
    ...
    assert validator.has_no_duplicate_values_generic(cat, 'uuid')
    ...
    loe = validator.find_values_by_name(good_target, 'uuid')
    assert len(loe) == 5
    assert validator.has_no_duplicate_values_by_name(good_target, 'uuid')
    ...
    assert not validator.has_no_duplicate_values_by_name(bad_target, 'uuid')
    ...
    assert not validator.has_no_duplicate_values_generic(bad_target_yaml, 'uuid')
...
def test_has_no_duplicate_values_pydantic():
    ...
    cat = catalog.Catalog.oscal_read(catalog_path)
    assert validator.has_no_duplicate_values_by_type(cat, catalog.Metadata)
    ...
    good_target_path = yaml_path / 'good_target.yaml'
    good_target = ostarget.TargetDefinition.oscal_read(good_target_path)
    assert not validator.has_no_duplicate_values_by_type(good_target, ostarget.Prop)
