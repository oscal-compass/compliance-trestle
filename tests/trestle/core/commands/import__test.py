...
def test_import_from_inside_trestle_project_is_bad(tmp_trestle_dir: pathlib.Path) -> None:
    ...
    sample_file.write('{}')
    sample_file.close()
    test_args = f'trestle import -f infile.json -o catalog'.split()
    with patch.object(sys, 'argv', test_args):
        try:
            Trestle().run()
        except Exception:
...
def test_import_bad_input_extension(tmp_trestle_dir: pathlib.Path) -> None:
    ...
    # trestle_tmp_dir is in /private/var/folders and has been initialized for trestle.
    # os.getcwd() is trestle_tmp_dir.
    # tmp_dir would be a relative path, inside trestle_tmp_dir as well.
    # tmp_file does not yet exist, but is found inside trestle_tmp_dir, which is not allowed.
    # Test
    # test_args = ['trestle', 'import', '-f', sys.executable, '-o', 'catalog']
...
def test_import_root_key_failure(tmp_trestle_dir):
    ...
    sample_file.write(b'{"id" : "0000", "title" : "nothing"}')
