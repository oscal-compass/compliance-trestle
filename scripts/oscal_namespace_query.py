...
def extract_queries(namespace_data, filter_document_type=None):
    ...
    for namespace_uri, ns_data in namespaces.items():
        for document_type, doc_data in ns_data.items():
            ...
            for _element, elem_data in doc_data.items():
...
def run_query(
    script_path, element, attribute='', conditions=None, namespace=None, document_type=None, constraint_key=None
):
    """
...
Note: When querying a main model, constraints from its imported common
    ...
    parser.add_argument('--document-type', '-d', help='Filter tests by document type')
