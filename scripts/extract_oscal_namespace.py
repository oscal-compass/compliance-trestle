...
def parse_metaschema(file_path):
    ...
    for allowed_values in root.findall('.//m:allowed-values', ns):
        ...
        for enum in allowed_values.findall('m:enum', ns):
            ...
            enum_data = {'value': value, 'description': description}
            if deprecated:
...
def main():
    ...
    metaschema_files = [f for f in metaschemas_dir.glob('oscal_*_metaschema_RESOLVED.xml') if 'complete' not in f.name]
