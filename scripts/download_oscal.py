...
def download_oscal(version, output_dir=None):
    ...
    zip_path = Path(f"/tmp/oscal-{version}.zip")
    extract_path = Path(f"/tmp/oscal-extract-{version}")
...
Available versions can be found at:
  https://github.com/usnistgov/OSCAL/releases
    ...
    parser.add_argument(
        '--version', '-v',
        help='OSCAL version to download (e.g., 1.2.2). If not specified, downloads latest.'
    )
