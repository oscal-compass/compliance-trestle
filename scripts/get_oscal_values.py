...
def find_matching_constraints(namespace_data, element, attribute, namespace_uri=None, conditions=None, document_type=None, constraint_key=None):
    """
...
Note: When specifying a document type, constraints from imported common
    ...
    parser.add_argument('attribute', nargs='?', default='',
                       help='Attribute name (e.g., name, value). Omit for field value constraints.')
    parser.add_argument('--condition', '-c', action='append',
                       help='Condition in format key=value (can be specified multiple times)')
    parser.add_argument('--namespace', '-n',
                       default='http://csrc.nist.gov/ns/oscal',
                       help='Namespace URI (default: OSCAL namespace)')
    parser.add_argument('--document-type', '-d',
                       help='Filter by document type (includes imported common modules)')
    parser.add_argument('--constraint-key', '-k',
                       help='Exact constraint key to match (e.g., metadata/prop[@name])')
