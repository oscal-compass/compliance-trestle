...
def init_arguments(cmd: Command) -> None:
    ...
    group.add_argument('-f', '--file', help='Name of file or model to validate.')
    group.add_argument('-t', '--type', choices=const.MODEL_TYPE_LIST, help='Validate all models of this type.')
    group.add_argument('-a', '--all', action='store_true', help='Validate all models in trestle directory.')
    cmd.add_argument('-i', '--item', choices=['uuid'], help='Name of item to validate.', required=True)
