...
class CsvToOscalComponentDefinition(TaskBase):
    ...
    def _get_implemented_requirement(
        ...
        description = self._resolved_profile_catalog_helper.get_title(control_id)
...
class _ResolvedProfileCatalogHelper():
    ...
    def __init__(self, profile_list: List[str], root: str = '.') -> None:
        ...
        for profile in self._profile_list:
            ...
            if profile not in self._catalog_map.keys():
                self._catalog_map[profile] = catalog
                self._catalog_interface_map[profile] = CatalogInterface(catalog)
    ...
    def get_title(self, control_id: str) -> str:
        """Get title for control_id."""
        rval = ''
        for profile in self._catalog_interface_map.keys():
            catalog_interface = self._catalog_interface_map[profile]
            control = catalog_interface.get_control(control_id)
            if control:
                title = control.title
                if not title:
                    continue
