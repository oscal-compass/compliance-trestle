...
class PoamValidator:
    ...
    def __init__(self, validate_mode: str = 'warn'):
        """
...
class PoamXlsxHelper:
    """Helper class for reading FedRAMP POAM Excel templates."""
    ...
    def __init__(self):
        ...
        self._header_row = 5  # FedRAMP template has headers at row 5 (1-indexed)
    ...
    def _map_columns(self) -> None:
        ...
        # Read header row (row 5 in FedRAMP template, 1-indexed)
    ...
    def parse_date(self, date_value: Any) -> Optional[datetime.datetime]:
        """
        Parse Excel date to datetime with timezone.

        Args:
            date_value: Excel date (datetime object or string)
...
class PoamBuilder:
    """Builder class for constructing OSCAL POAM objects from Excel data."""

    def __init__(self, timestamp: str, validator: PoamValidator):
        """
        Initialize builder.
    ...
    def create_poam_item(self, poam_id: str, row_data: Dict[str, Any]) -> PoamItem:
        ...
        """
        # Required fields
        title = row_data.get(PoamXlsxHelper.WEAKNESS_NAME, '').strip() if row_data.get(PoamXlsxHelper.WEAKNESS_NAME) else ''
        description = row_data.get(PoamXlsxHelper.WEAKNESS_DESCRIPTION, '').strip() if row_data.get(PoamXlsxHelper.WEAKNESS_DESCRIPTION) else ''
    ...
    def create_observation(self, poam_id: str, row_data: Dict[str, Any], helper: PoamXlsxHelper) -> Observation:
        ...
        Returns:
            Observation object
        """
        weakness_name = row_data.get(PoamXlsxHelper.WEAKNESS_NAME, '').strip() if row_data.get(PoamXlsxHelper.WEAKNESS_NAME) else ''
    ...
    def create_risk(self, poam_id: str, row_data: Dict[str, Any], helper: PoamXlsxHelper) -> Risk:
        ...
        statement = statement_raw.strip() if statement_raw and isinstance(statement_raw, str) else (statement_raw if statement_raw else description)
...
class XlsxToOscalPoam(TaskBase):
    """
    Transform FedRAMP POAM Excel spreadsheets to OSCAL POAM JSON.

    This task reads a FedRAMP POAM Excel template (specifically the
    ...
    Excel Requirements:
        - Must use FedRAMP POAM template structure
        - Headers at row 5
    ...
    See Also:
        ...
        - FedRAMP POAM Template documentation
    ...
    def print_info(self) -> None:
        ...
        logger.info('')
        logger.info('Purpose: Transform FedRAMP POAM Excel spreadsheets to OSCAL POAM JSON format.')
        logger.info('')
        logger.info('Configuration flags:')
        logger.info('  xlsx-file      = (required) Path to FedRAMP POAM Excel file')
    ...
    def _execute(self) -> TaskOutcome:
        ...
        # Write output
        output_file = output_path / 'poam.json'
