...
from trestle.oscal.mapping import MappingDescription
...
MAP_SOURCE_ID_REF_LIST = 'Map_Source_Id_Ref_List'
...
L_MAP_SOURCE_ID_REF_LIST = MAP_SOURCE_ID_REF_LIST.lower()
L_MAP_TARGET_ID_REF_LIST = MAP_TARGET_ID_REF_LIST.lower()
L_MAP_RELATIONSHIP = MAP_RELATIONSHIP.lower()
L_MAP_CONFIDENCE_SCORE = MAP_CONFIDENCE_SCORE.lower()
L_MAP_COVERAGE = MAP_COVERAGE.lower()
...
class _McMgr:
    ...
    def _parse_percentage(self, value: str) -> Optional[float]:
        """Parse percentage string to decimal (0-1)."""
        if not value:
            return None
        ...
        if result < 0 or result > 1:
            logger.warning(f'Percentage value out of range (0-1): {result}')
            return None
    ...
    def add_mapping(
        ...
        src_id_ref_list: List[str],
        tgt_id_ref_list: List[str],
        relationship_type: str,
        confidence_score_str: str,
        coverage_str: str,
        kvp_set: Dict,
    ) -> None:
        """Add mapping."""
        ...
        sources = []
        targets = []
        # Add all source items
        for src_id_ref in src_id_ref_list:
        ...
        # Create map with relationship
        map_ = Map(uuid=str(uuid.uuid4()), relationship=relationship_type, sources=sources, targets=targets)
