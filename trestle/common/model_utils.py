...
class ModelUtils:
    ...
    def load_distributed(
        ...
        Args:
            ...
            abs_trestle_root: The workspace root directory.
    ...
    def get_relative_model_type(relative_path: pathlib.Path) -> Tuple[Type[OscalBaseModel], str]:
        ...
        Args:
            relative_path: Relative path of the model with respect to the root directory of the workspace.
        Returns:
            Type of Oscal Model for the provided model
            Alias of that oscal model.
        """
        if len(relative_path.parts) < 2:
            raise TrestleError('Insufficient path length to be a valid relative path w.r.t workspace root directory.')
