...
from trestle.oscal.common import ImplementedComponent, InventoryItem, Property, PropertyList, SubjectReference
...
class OscalFactory():
    ...
    def _batch_observations(self, index):
        ...
        for i in range(start, end):
            ...
            propertyList.items.append({'name': 'Check_ID', 'value': rule_use.check_id, 'ns': self._ns})
            propertyList.items.append({'name': 'Check_ID_Benchmark', 'value': rule_use.check_id_benchmark, 'ns': self._ns, 'class':'scc_predefined_profile'})
            propertyList.items.append({'name': 'Check_ID_Version', 'value': rule_use.check_id_version, 'ns': self._ns, 'class': 'scc_predefined_profile_version'})
            propertyList.items.append({'name': 'Check_ID_Level', 'value': rule_use.check_id_level, 'ns': self._ns})
            propertyList.items.append({'name': 'Rule_ID', 'value': rule_use.rule_id, 'ns': self._ns, 'class': 'scc_goal_description'})
            propertyList.items.append({'name': 'Rule_ID', 'value': rule_use.rule_id, 'ns': self._ns, 'class': 'scc_check_name_id'})
            propertyList.items.append({'name': 'State', 'value': rule_use.state, 'ns': self._ns, 'class': 'scc_result'})
            propertyList.items.append({'name': 'Timestamp', 'value': rule_use.timestamp, 'ns': self._ns, 'class': 'scc_timestamp'})
            observation.props = propertyList
