...
class OscalPoamPlanOfActionAndMilestones(OscalBaseModel):
    ...
    import_ssp: Optional[OscalAssessmentCommonImportSsp] = Field(
        None, alias='import-ssp'
    )
    system_id: Optional[OscalImplementationCommonSystemId] = Field(
        None, alias='system-id'
    )
    local_definitions: Optional[OscalPoamLocalDefinitions] = Field(
    ...
    poam_items: List[OscalPoamPoamItem] = Field(..., alias='poam-items', min_items=1)
    back_matter: Optional[OscalMetadataBackMatter] = Field(None, alias='back-matter')


class Model(OscalBaseModel):
    plan_of_action_and_milestones: OscalPoamPlanOfActionAndMilestones = Field(
