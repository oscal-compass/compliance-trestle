...
- Fetch sample trestle tanium-result-to-oscal config file. It informs the trestle command where to read input and write output.

```bash
(venv.trestle)$ curl 'https://raw.githubusercontent.com/IBM/compliance-trestle/develop/tests/data/tasks/tanium/demo-tanium-result-to-oscal.config' > tanium/demo-tanium-result-to-oscal.config
```

<details>
<summary>sample: demo-tanium-result-to-oscal.config</summary>
...
(venv.trestle)$ trestle task tanium-result-to-oscal -c demo-tanium-result-to-oscal.config 
input: tests/data/tasks/tanium/input/Tanium.doc-json
output: tests/data/tasks/tanium/runtime/Tanium.oscal.json
inventory: 1
observations: 9
Task: tanium-result-to-oscal executed successfully.
```
